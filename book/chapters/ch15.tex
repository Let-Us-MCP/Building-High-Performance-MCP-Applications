...
Meridian's 134 tests run in 2.3 seconds and use no model at all. The eval suite
is separate, slower, and gates deploys rather than commits.
...
Note \texttt{raw\_post}. Not the client. A hand-built HTTP request, because
testing header validation through a client that constructs correct headers tests
nothing.
...
non-deterministic, and their result is a score rather than a pass or fail.
...
\textbf{Assert on structure, not prose.} ``The answer contains 55.4'' is stable.
...
\midrule
Unit and contract & 134 tests, no model & every commit \\
