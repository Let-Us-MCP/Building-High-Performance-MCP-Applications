...
The trick in agentic systems is subtler. Nobody is convinced attacks do not
exist. People are convinced that \emph{their} agent is safe because the servers
are internal, the tools are read-only, and there is a human in the loop.

Each of those is a reasonable-sounding sentence and each has a hole in it. This
chapter is about the holes.
...
Now the frame that makes the rest tractable.
...
\bottomrule
\end{tabularx}
\caption{The fourth row is the one that makes the others survivable, which is
why Chapter~\ref{ch:apps} states it as a rule rather than a recommendation.}
...
from the bottom, and that the bottom row is detective rather than preventive.}
