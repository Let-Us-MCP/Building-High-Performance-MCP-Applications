...
This is the chapter the rest of the book has been setting up: a catalogue of
techniques, each with a measurement attached, each with an honest note about the
conditions under which it does nothing.

One ground rule first, because it is the whole discipline. Every change gets
attributed individually. Not ``we did six things and it got faster'', which
tells you nothing and cannot be selectively undone, but six numbers, one per
change, including the changes that turned out to be worth nothing at all.
Chapter~\ref{ch:measuring} built the instrument; this chapter uses it.
...
Four changes I measured that returned nothing, or close enough to nothing that
the effort was wasted. Every one of them is something I would have recommended
on intuition.
