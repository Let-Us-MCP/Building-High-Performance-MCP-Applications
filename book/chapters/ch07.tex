...
simple. The interesting content is organisational rather than technical.
...
Messages, plural, with roles. That matters more than it looks. A prompt is not a
string you interpolate; it is a conversation prefix. You can seed it with a user
turn, an assistant turn demonstrating the desired shape, and another user turn,
...
pattern, and it is why prompt names should read like commands rather than like
function identifiers.

\begin{notebox}{``User-controlled'' is about invocation, not authorship}
...
Meridian's compliance server knows things the host does not: that a Suspicious
Activity Report is due within 30 days of \emph{detection} rather than
escalation, that corridor risk is rated on counterparty jurisdiction rather than
booking branch, that a flagged transaction needs a named officer.
...
One \texttt{compliance-review} prompt on the compliance server fixed it. Not
because the prompt was better than the three it replaced, but because there was
one of it.
...
Note the prefix. \texttt{com.meridian/} is legal because the second label is
\texttt{meridian}, not \texttt{modelcontextprotocol} or \texttt{mcp}. Prefixes
...
the data, or somebody's Monday edit to the blueprint. The version in the audit
log answers that in five seconds instead of a day.
...
unit tests and they catch most regressions, because most regressions are
somebody breaking the template rather than the model changing its mind.
...
\texttt{meridian/host/model.py}. Small, which is the point: unlike tool
descriptions, a rendered prompt is paid once per invocation rather than on every
turn.
\end{measurebox}
...
\textbf{Names the tool to call.} Not ``review the transaction'' but ``call
\texttt{review\_transaction}''. Removes a planning step and a chance to guess
wrong.

\textbf{Says ``exactly''.} Compliance outcomes must not be paraphrased. This is
a domain constraint the host could not know.

\textbf{Inoculates against injection.} That sentence about guidance being data
rather than instructions is defence in depth, sitting in the blueprint where it
...
arguments. Meridian's \texttt{credit-review} takes an \texttt{audience} argument
rather than shipping \texttt{credit-review-committee} and
\texttt{credit-review-rm}.
...
producing a review template. Chapter~\ref{ch:mrtr} covers the mechanism in full,
and the cost analysis there applies unchanged: an elicitation here costs a round
trip plus a model turn, so gathering it as a prompt \emph{argument} is nearly
always better. Arguments are free; questions are not.
...
\item Rendered prompts cost once per invocation, unlike tool descriptions.
Compress the list response, not the template.
\item Stable content first, variable content last, for prompt-cache hits.
\item Name the tools, bound the output, and keep thresholds in code rather than
in prose.
\end{itemize}
