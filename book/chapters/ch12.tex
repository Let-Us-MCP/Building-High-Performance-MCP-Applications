...
the only party in a position to have prevented it. That makes it the accountable component whether or not anybody designed it to
be, which is worth deciding deliberately before an incident decides it for you.
...
\subsection{Where the label comes from}

The obvious implementation is to namespace on something the server tells you,
and the specification closes that door explicitly: \texttt{serverInfo.name} is
not guaranteed unique across servers and must not be used to disambiguate.

The reason is worth sitting with, because the failure is quiet. Two vendors each
ship something reasonably called \texttt{search-server}. A host that namespaces
on the reported name now has two servers claiming the prefix
\texttt{search-server}, and the merged catalogue contains
\texttt{search-server.query} twice. One of them wins. Which one depends on
dictionary ordering, so it varies between restarts, and the symptom a user
reports is that a tool sometimes talks to the wrong system.

The label therefore has to be assigned by the host, from configuration, and it
has to be stable across restarts because it ends up in conversation history and
audit records. Meridian takes it as the first argument to \texttt{connect} for
exactly that reason: it is a decision the operator makes once, not a value a
server gets to influence.
...
This is the same principle as \texttt{clientInfo} in
Chapter~\ref{ch:architecture} and the annotations in
Chapter~\ref{ch:tools}, arriving for the third time: anything the other party
reports about itself is useful for display and unusable as a key. Here the key
is a routing decision, and routing on a self-reported string means a server can
choose which of your other servers it collides with.
