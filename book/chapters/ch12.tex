...
\epigraph{``I'm the guy doing his job. You must be the other guy.''}{Sean
Dignam, \emph{The Departed} (2006)}

Dignam's whole character is about who is actually accountable when several
parties are nominally responsible. Which is the host's problem exactly. The model
suggests, the server executes, the user watches, and when something goes wrong
the host is the guy doing his job or the other guy. There is no third option,
because the host is the only component that can see all of it.

Building a server is mostly craft. Building a host is mostly judgement, and this
chapter is about the five judgements that matter.
...
Here is the job nobody assigns to anybody, so it does not get done.
...
\textbf{A denial returns a tool execution error, not an exception.} The model
should learn that the call was refused and adapt, rather than have the loop
crash. ``The user declined'' is information the model can use.
...
The hardest UX problem in this book.
...
The default declines. That is deliberate: a host that has not been wired up
should refuse rather than invent answers on the user's behalf.
...
\textbf{Tell the model, not just the log.} If a capability the user asked for is
unavailable, that belongs in the context so the model can say so rather than
looping.
...
W3C trace context in \texttt{\_meta}, under the reserved
\texttt{traceparent} key. One line, and your host-to-server spans join up in
whatever tracing backend you already run.
