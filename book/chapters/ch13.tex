...
\epigraph{``What we do in life echoes in eternity.''}{Maximus,
\emph{Gladiator} (2000)}

Or, in the agentic case, it echoes in the context window, which is worse,
because eternity is free and the context window is billed per token on every
subsequent turn.

This chapter is about the loop: how it terminates, where state lives now that
the protocol holds none, and how to stop the context filling with the corpses of
completed steps.
...
Note that a budget stop still produces an \emph{answer}. Not an exception, not a
blank. The user asked a question and deserves to be told why they are not getting
one.
...
Refetch and retry once, rather than reporting failure. The specification
...
what you already read}, and the lever is context management rather than
prompting.
...
The measurement that matters is the quality/cost frontier, and the honest way to
find it is to run your eval suite at each mix rather than to reason about it. It
...
that faster. You can make it \emph{feel} faster, and that is not a trick, it is
