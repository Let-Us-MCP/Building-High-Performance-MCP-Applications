...
Keycloak, or your own. MCP specifies how a client \emph{finds} it, not how it
works.
\end{description}
...
\vspace{6pt}
\footnotesize
Modelled at 40\,ms per hop, not measured; Meridian's benchmark auth is local and
would flatter these badly. The number that matters is the round-trip count,
which is exact.
\end{measurebox}
...
\keyidea{Steps 1 through 3 are properties of the server, not of the request.
Cache them per origin, for hours. The token is a property of the user and the
scope set. Cache it until it expires, and refresh it before it does, not after.}
...
and it fails in confusing ways rather than clean ones.
\end{legacybox}

\section{Two validations that stop real attacks}

Most of OAuth's complexity is scar tissue. Two pieces of it are worth
understanding rather than merely implementing.
...
Note 403 rather than 401. A 401 means ``I do not know who you are''; a 403 means
``I know exactly who you are and you may not do that''. Clients branch on this,
...
Better still, do not get there. Refresh proactively when a token is within some
margin of expiry, so the refresh happens between loop iterations rather than
inside one.
...
\item Expired tokens are a recoverable interruption, not a failed task. Bound
your retries.
