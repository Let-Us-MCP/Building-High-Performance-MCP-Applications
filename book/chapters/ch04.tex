...
the structure \emph{High Performance Browser Networking} used for TLS and for
the same reason. Authorization is not a policy you configure once and stop
thinking about. It is a sequence of network round trips, all of which happen
before your application does any useful work, and each of which the user
experiences as your application being slow to start.

Done properly you pay that sequence once and cache the result for hours. Done
carelessly you pay it on every request forever, and because each individual
round trip looks cheap in isolation, nobody notices until the total is the
largest single item in your latency budget.
