...
system he was operating inside. He did not, and people died. Every architecture
chapter should open with a warning like that, because architecture is exactly
the thing you think you understand right up until it bites.

The subtlety in MCP is this. It looks like RPC. It is shaped like RPC. Requests
go out, responses come back, and if you squint it is just a slightly opinionated
JSON API. If you treat it that way you will build something insecure, because
the interesting part of the architecture is the trust boundary, and the trust
boundary sits somewhere most people do not guess.

This chapter builds the model from nothing. Three roles, then the thing that
changed in 2026, then the contract that tells you how long any of this will
remain true.
...
Now the change, and it is genuinely large.
...
Honesty time, because this is not free and the costs come in two very different
sizes.
...
Let me make that concrete, because the abstract version does not change
anybody's design.
