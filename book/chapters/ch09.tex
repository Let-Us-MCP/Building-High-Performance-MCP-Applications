...
\epigraph{``I'll be back.''}{The Terminator, \emph{The Terminator} (1984)}

The whole appeal of that line is the confidence of the deferral. Not ``wait
here''. Not ``I will hold this door open indefinitely''. Just: this is going to
take a while, and I will return.

That is the Tasks extension in one sentence. And Tasks arriving as an extension
rather than as core protocol is the other half of this chapter, because the
extensions framework is how MCP now grows without breaking.
...
\textbf{The core stays small.} Tasks moved \emph{out} of core in this revision.
That is a protocol getting smaller, which almost never happens, and it happened
because the extensions framework gave it somewhere to go.
...
prefix. If it turns out to be generally useful, the road to standardisation runs
through a SEP rather than through a fait accompli.
...
Task status is polled, or pushed via \texttt{notifications/tasks} to clients who
subscribed. Different lifetime, different channel, and mixing them up produces a
client that waits for progress on a request that ended twenty minutes ago.
\end{notebox}
