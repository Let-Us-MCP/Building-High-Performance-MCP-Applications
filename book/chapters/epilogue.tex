...
Deleting the handshake was not a tidying-up exercise. It was a decision that MCP
servers should be ordinary web services, deployable on the same infrastructure,
scaled by the same tools, operated by the same people.

Everything followed from it. No sessions, no resumability, no server-initiated
requests, MRTR instead of callbacks, explicit handles instead of implicit state.
...
My read is that these settle rather than merge. MCP owns the agent-to-capability
...
another extension is, honestly, a coin flip, and it will be decided by which
ecosystem gets there first rather than by which design is better.
...
The honest position is that this is a property of the technology rather than a
bug in anyone's implementation. Language models process instructions and data in
one stream. Until that changes, or until something at the model layer provides a
real boundary, the trifecta framing is the best tool available, and it is a
mitigation strategy rather than a solution.
...
\textbf{The host will still be the trust boundary.} It follows from the
architecture rather than from a version. Any design where the model is the
...
\texttt{2026-07-28}, and \texttt{meridian/protocol/} will be a teaching artefact
rather than a practical necessity. Use the SDKs. Read the wire when something is
...
\texttt{meridian/protocol/legacy.py} and enjoy the diff. Building it as a
separate wrapper rather than as patches to the core was entirely so that day is a
deletion rather than an archaeology project.
