...
An MCP App is a curtain. The user sees a button; the server decides what the
button does and what the button says it does. Those are two separate things
under one party's control, which is a sentence worth reading twice.

That tension runs through this whole chapter. Interactive interfaces are
genuinely the right answer for some tool results and genuinely dangerous, and
both facts come from the same source: the HTML arrived from somebody else's
server. So: when an app beats a paragraph, and what the host has to do to make
rendering one safe.
...
\item[Prefetch] Fetch and parse the template during idle time, so the first
render is instant rather than a fetch plus a parse plus a paint.
...
Network access is denied by default and granted only to origins the server
declared in \texttt{\_meta.ui.csp}. An app that wants to load a charting library
from a CDN must say so in advance, in a field a human can read during review.
...
which is the Apps extension's own revision rather than core MCP's. Extensions
...
in \texttt{@modelcontextprotocol/ext-apps} is a convenience wrapper rather than a
requirement. It is postMessage and JSON-RPC underneath, which means the whole
...
return useful \texttt{content}, and it must be useful on its own rather than a
placeholder saying ``see the dashboard''. Meridian's tools all return complete
...
\item Sandboxed iframe. No parent DOM, no cookies, no network unless the server
declared origins.
...
\item \texttt{structuredContent} feeds the app, \texttt{content} feeds the
model, and that split is how an app saves context.
