...
covers the caching model; here the relevant fact is that a well-behaved client
fetches your catalogue once per TTL rather than once per task.
...
Note that the result carries the same data twice. That is deliberate and
recommended: \texttt{structuredContent} for anything that will parse it,
...
validates \texttt{structuredContent} before it ever reaches the model, and a
malformed response is caught at the boundary instead of confusing the planner
two turns later.
...
issued, whatever order they complete in. Meridian iterates futures in submission
order rather than as-completed, for exactly this reason.
...
fraud & \texttt{explain\_signal}   & So the model can explain rather than guess \\
