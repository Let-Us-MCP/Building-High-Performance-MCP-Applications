...
\epigraph{``It's about getting things down to one number.''}{Peter Brand,
\emph{Moneyball} (2011)}

Brand's point was not that one number is enough. It was that an argument
conducted in adjectives cannot be settled, and an argument conducted in numbers
can. Everyone in that room had been scouting on instinct for thirty years and
each of them was certain.

This is the chapter the book was written for. Everything before it was
mechanism. Everything after it is optimisation, and optimisation without
measurement is superstition with a build pipeline.
...
\textbf{Transport and execution are separate spans.} That is the subtraction
above, done automatically for every call rather than once in a benchmark.
