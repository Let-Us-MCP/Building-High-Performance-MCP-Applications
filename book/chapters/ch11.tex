...
The pattern from Chapter~\ref{ch:architecture}, built properly.
...
# --> tools/call, later, possibly to a different replica
{ "name": "add_item", "arguments": { "basket_id": "bsk_a1b2c3", "sku": "..." } }
\end{py}

From the wire's point of view, \texttt{basket\_id} is a string argument. There is
no protocol concept of a handle, which is exactly why it works on every client
and every transport with no negotiation.

Four rules, and each of them is somebody's incident report:
...
Keep it opaque
  & \texttt{basket\_user42\_20260728} invites enumeration of user 43 \\
State the lifetime in the creation tool's description
...
Re-check authorization against it on every single call. For an unauthenticated
server the handle \emph{is} a bearer token, so give it real entropy and a
bounded life.}
...
\begin{py}
{
    ...
    "properties": {
        ...
        "idempotencyKey": {
            "type": "string",
            "description": "Client-generated unique id. Repeating a key "
                           "within 24 hours returns the original result "
...
Server side, the shape is always the same: look up the key, return the stored
result if present, otherwise do the work and store it before returning. The
description matters as much as the mechanism, because the model has to know to
generate a fresh key per intent and reuse it per retry.
...
Meridian has 134 tests, and the ones in this section are the ones I would write
...
\texttt{subTest} means one failing method reports as one failure rather than
aborting the loop, so you see all six.
...
\begin{enumerate}[leftmargin=1.6em, itemsep=3pt]
\item \textbf{Construct}, with instructions written for a model rather than for
a README.
