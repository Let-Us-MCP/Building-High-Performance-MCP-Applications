...
Andy's plan worked because he did the same small thing every night for nineteen
years. Operations is that. Not heroics at three in the morning, but the
unglamorous compounding of health checks that mean something, drains that
actually drain, and alerts that fire before the user notices.

The good news for MCP specifically: the stateless core deleted most of the hard
parts of this chapter before it was written.
...
The default for anything shared.
...
def healthy(server) -> tuple[bool, dict]:
    ...
    A process can accept connections while its catalogue is empty, its
    dependencies are down, or its handlers all raise. Answering discovery
    proves routing, capability derivation, and serialisation all work.
    """
    probe = {
    ...
    resp = server.handle(probe)
    if "error" in resp:
        return False, {"reason": resp["error"]["message"]}
    result = resp["result"]
    return (
        PROTOCOL_VERSION in result.get("supportedVersions", [])
...
The dual-era bridge answers \texttt{ping} for exactly this reason, which is a
compatibility shim rather than a recommendation.
\end{notebox}

\section{Graceful drain}

Statelessness makes this easy, and there is still a right order.
...
Do it before production does it for you, and test the right thing.
...
What you want written down before you need it.
