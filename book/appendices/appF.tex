...
mechanically. \texttt{check\_\allowbreak contrastive.py} exists because a first
draft leaned on ``X is not Y, it is Z'' 244 times.
\texttt{check\_\allowbreak listings.py} exists because the claim that every
listing is real code deserved enforcement, and good intentions are not
enforcement. Appendix~\ref{ch:companion} covers running them.
